...
export default function Pricing() {
  ...
  useEffect(() => { load(); }, []);
  const load = async () => {
    const { data: plansData } = await supabase.from('plans').select('*').eq('active', true).order('display_order');
    setPlans(plansData || []);
    const { data: feats } = await supabase.from('plan_features').select('plan_id,key,value');
